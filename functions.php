<?php
/**
 * 4w4-theme-dmp functions and definitions
 *
...
if ( ! function_exists( 'theme_dmp_setup' ) ) :
	/**
	 ...
	 */
	function theme_dmp_setup() {
		/*
		 * Make theme available for translation.
		 * Translations can be filed in the /languages/ directory.
		 * If you're building a theme based on 4w4-theme-dmp, use a find and replace
		 * to change 'theme-dmp' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'theme-dmp', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'theme-dmp' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'theme_dmp_custom_background_args',
				array(
...
add_action( 'after_setup_theme', 'theme_dmp_setup' );
 ...
 */
function theme_dmp_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'theme_dmp_content_width', 640 );
}
add_action( 'after_setup_theme', 'theme_dmp_content_width', 0 );
 ...
 */
function theme_dmp_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'theme-dmp' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'theme-dmp' ),
	...
	register_sidebar(
		array(
			'name'          => esc_html__( 'Footerbar', 'theme-dmp' ),
			'id'            => 'footer-1',
			'description'   => esc_html__( 'Add widgets here.', 'theme-dmp' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
...
add_action( 'widgets_init', 'theme_dmp_widgets_init' );
...
function theme4w4_scripts() {
	...
	wp_register_script( 'theme4w4-rest-api', get_template_directory_uri() . '/js/rest-api.js', array(), filemtime(get_template_directory() . "/js/rest-api.js"), true );
